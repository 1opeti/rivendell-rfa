...
//
//      $Id: cae_alsa.cpp,v 1.41.2.1 2009/04/27 14:38:09 cvs Exp $
//
  ...
  while(!alsa_format->exiting) {
	...
	case SND_PCM_FORMAT_S16_LE:
	  ...
	  for(unsigned i=0;i<alsa_format->channels;i+=2) {
		...
		if(((short *)alsa_format->
		    card_buffer)[alsa_format->channels*k+2*i+1+j]>
		   out_meter[i+1][j]) {
		  out_meter[i+1][j]=
		    ((short *)alsa_format->
		     card_buffer)[alsa_format->channels*k+2*i+1+j];
		...
		addValue(((double)out_meter[i][j])/32768.0);
	      alsa_output_meter[alsa_format->card][i+1][j]->
		addValue(((double)out_meter[i+1][j])/32768.0);
	    }
	  }
	  break;
	...
	case SND_PCM_FORMAT_S32_LE:
	  ...
	  for(unsigned i=0;i<alsa_format->channels;i+=2) {
	    for(unsigned j=0;j<2;j++) {
		  ...
		  k++) {
		if(((short *)alsa_format->
		    card_buffer)[alsa_format->channels*2*k+4*i+1+2*j]>
		   out_meter[i][j]) {
		  out_meter[i][j]=
		    ((short *)alsa_format->
		     card_buffer)[alsa_format->channels*2*k+4*i+1+2*j];
		...
		if(((short *)alsa_format->
		    card_buffer)[alsa_format->channels*2*k+4*i+3+2*j]>
		   out_meter[i+1][j]) {
		  out_meter[i+1][j]=
		    ((short *)alsa_format->
		     card_buffer)[alsa_format->channels*2*k+4*i+3+2*j];
