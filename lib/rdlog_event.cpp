...
//
//      $Id: rdlog_event.cpp,v 1.93.2.1 2009/02/12 14:21:26 cvs Exp $
//
...
%s.DUCK_UP_GAIN,%s.DUCK_DOWN_GAIN \
from %s left join CART on %s.CART_NUMBER=CART.NUMBER order by COUNT",
  ...
  for(int i=0;i<q->size();i++) {
    ...
    line.setStartTime(RDLogLine::Imported,q->value(2).toTime()); // Start Time
    line.setStartTime(RDLogLine::Logged,q->value(2).toTime());
    line.
    ...
    line.setLinkStartTime(q->value(48).toTime());           // Link Start Time
...
void RDLogEvent::SaveLine(int line)
{
  ...
  sql=QString().sprintf("insert into %s set ID=%d,COUNT=%d,\
                         CART_NUMBER=%u,START_TIME=\"%s\",TIME_TYPE=%d,\
                         ...
                         LINK_EVENT_NAME=\"%s\",LINK_START_TIME=\"%s\",\
