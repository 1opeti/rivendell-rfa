...
//
//      $Id: rdcart.cpp,v 1.61.2.3 2009/03/24 12:11:18 cvs Exp $
//
...
#ifndef WIN32
/*  Random rotation disabled -- FFG 4/2/2008
  switch(playOrder()) {
      case RDCart::Random:
	q->seek(-1);
	while(q->next()) {
	  if((q->value(2).toDouble()/q->value(1).toDouble())==play_ratio) {
	    eligibles.push_back(q->at());
	  }
	}
	if(eligibles.size()>0) {
	  q->seek(eligibles[(int)((double)eligibles.size()*
