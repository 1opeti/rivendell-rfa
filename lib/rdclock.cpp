...
//
//      $Id: rdclock.cpp,v 1.19.2.1 2008/11/30 00:32:56 fredg Exp $
//
...
bool RDClock::load()
{
  ...
  while(q->next()) {
    ...
    clock_events.back().setStartTime(q->value(1).toTime());
...
bool RDClock::save()
{
  ...
  for(unsigned i=0;i<clock_events.size();i++) {
    sql=QString().sprintf("insert into %s_CLK set EVENT_NAME=\"%s\",\
                           START_TIME=\"%s\",LENGTH=%d",
			  (const char *)clock_name_esc,
			  (const char *)clock_events[i].name(),
			  (const char *)clock_events[i].startTime().
			      toString("hh:mm:ss"),
