...
//
//      $Id: list_log.cpp,v 1.100.2.3 2009/03/26 20:27:57 cvs Exp $
//
...
void ListLog::RefreshItem(RDListViewItem *l,int line)
{
  ...
  switch(log_line->timeType()) {
      case RDLogLine::Hard:
	l->setText(1,QString("T")+log_line->startTime(RDLogLine::Logged).
		   toString(list_time_format));
	l->setText(8,QString("T")+log_line->startTime(RDLogLine::Logged).
		   toString(list_time_format));
	...
	if(!log_line->startTime(RDLogLine::Logged).isNull()) {
	  l->setText(8,log_line->startTime(RDLogLine::Logged).
		     toString(list_time_format));
	}
	else {
	  l->setText(8,"");
  ...
  for(int i=0;i<(list_log_list->childCount()-1);i++) {
    ...
    if((logline=list_log->logLine(line))!=NULL) {
	  ...
	  case RDLogLine::Paused:
		...
		case RDLogLine::Hard:
		  next->setText(1,QString(tr("T"))+logline->
		...
		default:
		  if(!logline->startTime(RDLogLine::Predicted).isNull()) {
		    next->setText(1,logline->startTime(RDLogLine::Predicted).
				  toString(list_time_format));
		  }
		  else {
		    next->setText(1,"");
...
*/
	  ...
	  default:
	    next->setText(1,logline->startTime(RDLogLine::Actual).
		      toString(list_time_format));
	    break;
      }
      next=(RDListViewItem *)next->nextSibling();
